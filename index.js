...
// 👇 Firebase Rules Bypass karne ke liye Master Key 👇
...
// ==========================================
// 3. NAYA USER REGISTER KARNA (With Secret)
// ==========================================
app.post('/api/user/register', async (req, res) => {
    try {
        ...
        if (existingUser) {
            await fetch(`${FIREBASE_DB_URL}/users/${phone}.json?auth=${FIREBASE_DB_SECRET}`, { method: 'PATCH', body: JSON.stringify({ email: safeEmail, name: name }) });
        ...
        const newUser = { name: name.trim(), email: safeEmail, savedVillage: "", savedStreet: "", referCode: newCode, freeDeliveries: 0, rewardExpiry: null };
        if (referrerPhone) { newUser.referredBy = referrerPhone; newUser.referralStatus = "pending"; }

        await fetch(`${FIREBASE_DB_URL}/users/${phone}.json?auth=${FIREBASE_DB_SECRET}`, { method: 'PUT', body: JSON.stringify(newUser) });
        await fetch(`${FIREBASE_DB_URL}/referCodes/${newCode}.json?auth=${FIREBASE_DB_SECRET}`, { method: 'PUT', body: JSON.stringify(phone) });
        ...
        if (referrerPhone) {
            ...
            await fetch(`${FIREBASE_DB_URL}/users/${referrerPhone}.json?auth=${FIREBASE_DB_SECRET}`, { method: 'PATCH', body: JSON.stringify({ freeDeliveries: currentDel + 3, rewardExpiry: expiryDate }) });
        }
...
// ==========================================
// 4. ORDER PLACE KARNA (Foolproof Email + Secret)
// ==========================================
app.post('/api/order/place', async (req, res) => {
    try {
        ...
        await fetch(`${FIREBASE_DB_URL}/orders/${orderId}.json?auth=${FIREBASE_DB_SECRET}`, { method: 'PUT', body: JSON.stringify(orderData) });
        ...
        await fetch(`${FIREBASE_DB_URL}/users/${customerDetails.phone}.json?auth=${FIREBASE_DB_SECRET}`, { method: 'PATCH', body: JSON.stringify(userUpdates) });
...
// ==========================================
// 5. ORDER CANCEL (Strict Email Validation + Secret)
// ==========================================
app.post('/api/order/cancel', async (req, res) => {
    try {
        ...
        if (!orderId || (!email && !phone)) {
            return res.json({ success: false, message: "Order ID aur Verification zaroori hai." });
        }
        ...
        // Agar phone aur email dono match nahi karte, toh block kardo
        if (orderData.phone !== phone && dbEmail !== reqEmail) {
        ...
        if (orderData.phone) {
            const userRes  = await fetch(`${FIREBASE_DB_URL}/users/${orderData.phone}.json?auth=${FIREBASE_DB_SECRET}`);
            const userData = await userRes.json() || {};
            await fetch(`${FIREBASE_DB_URL}/users/${orderData.phone}.json?auth=${FIREBASE_DB_SECRET}`, {
                method: 'PATCH',
